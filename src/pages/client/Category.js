...
const Category = () => {
  ...
  const handleLetterClick = (letter) => {
    ...
    if (letter === 'all') {
      // Fetch all items without letter filter
  ...
  const fetchItems = async (selectedLetter) => {
    try {
      ...
      // Fetch first video for each category
      for (const doc of querySnapshot.docs) {
        const data = { id: doc.id, ...doc.data() };
        if (data.videoCount && data.videoCount > 0) {
          const firstVideoId = await fetchFirstVideo(data.name);
  ...
  // Fetch first video for a category
  const fetchFirstVideo = async (categoryName) => {
    try {
          ...
          {items.map((item) => {
            ...
            return (
              <Link 
                ...
                to={hasVideos 
                  ? (type === 'pornstar' 
                    ? `/video/${item.firstVideoId || ''}` 
                    : `/category/videos/${item.name}`)
                  : `/${type}/${item.name}`}
              ...
              >
                ...
                <div className="flex items-center mt-2">
                  ...
                  <span className={`ml-auto ${hasVideos ? 'text-primary-400' : 'text-primary-500'} opacity-0 group-hover:opacity-100 transition-opacity duration-300`}>
                    {hasVideos ? 'Watch →' : 'View →'}
                  </span>
                </div>
              </Link>
            );
