...
const Categories = () => {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-6">Kategori Yönetimi</h1>
      ...
      {loading && categories.length === 0 ? (
            ...
            <table className="min-w-full divide-y divide-dark-600">
              ...
              <tbody className="bg-dark-700 divide-y divide-dark-600">
                {filteredCategories.map(category => (
                  <tr key={category.id} className="hover:bg-dark-600">
                    <td className="px-6 py-4 whitespace-nowrap">
                      {editingCategory === category.id ? (
                        <input
                          type="text"
                          className="w-full py-1 px-2 bg-dark-800 border border-dark-600 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 text-white"
                          value={editValue}
        ...
        <div className="bg-dark-700 rounded-lg p-8 text-center">
          <p className="text-gray-400">
            {searchQuery ? 'Arama kriterlerine uygun kategori bulunamadı.' : 'Henüz hiç kategori bulunmamaktadır.'}
          </p>
        </div>
      )}
    </div>
